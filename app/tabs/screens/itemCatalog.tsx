...
export default function ItemsCatalog() {
  ...
  const filteredItems = items?.filter(
    (item) =>
      item.itemName.toLowerCase().includes(search.toLowerCase()) || (item.sku && item.sku.includes(search))
  );
  ...
  const renderItem = ({ item }: { item: Item }) => (
    ...
    >
      <View style={{ flex: 1 }}>
        ...
        {item.sku ? <Text style={[style.text, { fontSize: 12, opacity: 0.7 }]}>SKU: {item.sku}</Text> : null}
      </View>
